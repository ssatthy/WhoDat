...
// duplicate for development environment

exports.sendPushNotificationInDev = functions.database.ref(development+ '/messages/{id}')
.onCreate(event => {
          const messageId = event.params.id;
          const messagePromise = admin.database().ref(development+ '/messages/'+ messageId).once('value');
          
          return Promise.all([messagePromise])
          .then(results => {
                const message = results[0].val();
                const tokenPromise = admin.database().ref(development+ '/users/'+ message.toId + '/token').once('value');
                const fromUserPromise = message.fromId.length < message.toId.length ?
                admin.database().ref(development+ '/anonymous-users/'+ message.fromId + '/' + message.toId).once('value')
                :
                admin.database().ref(development+ '/users/'+ message.fromId + '/name').once('value');
                
                const readPromise = admin.database().ref(development+ '/last-user-message-read/'+ message.toId).once('value');
...
exports.sendChatEndNotificationInDev = functions.database.ref(development+ '/chat-ended/{uid}')
.onCreate(event => {
          const toId = event.params.uid;
          const code = Object.keys(event.data.val())[0];
          const fromUser = event.data.val()[code];
          ...
          const toUser = admin.database().ref(development+ '/users/'+ toId + '/token').once('value');
          const ref = event.data.ref;
          return Promise.all([toUser, ref.remove()])
          .then(results => {
                const token = results[0].val();
